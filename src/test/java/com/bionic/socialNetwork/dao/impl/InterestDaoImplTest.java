package com.bionic.socialNetwork.dao.impl;
    ...
    public class InterestDaoImplTest {
        ...
        public void selectByInterest() throws Exception {
            List<User> user = interestDao.selectByInterest(interest);
            if (user.size() != 0) {
                assertEquals(login, user.get(0).getLogin());
            }
        }

        @After
        public void testDelete() throws Exception{
            Interest interest = interestDao.selectById(id);
            User user = (User) interestDao.selectByInterest(interest);
            userDao.delete(user);
